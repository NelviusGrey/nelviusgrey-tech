...
export function SectorSwitcher({ showBrandMedia = false }: { showBrandMedia?: boolean }) {
    ...
    <div>
      <div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr]">
        ...
        {sectorSolutions.map((sector, index) => {
          ...
          return (
            <button
              ...
              onPointerEnter={() => setActiveSlug(sector.slug)}
              onFocus={() => setActiveSlug(sector.slug)}
              data-cursor="Open"
              className={cn(
        ...
        <div className="relative mt-8 flex flex-col gap-4 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link prefetch={false} data-cursor="Discuss" href="/contact" className="group inline-flex items-center gap-2 text-sm font-semibold text-[color:var(--brand-green)]">
            Discuss this sector
